import { escapeXml, isVoucherBalanced, buildTallyVoucherXml, TallyVoucherInput } from '../src/modules/export/tally-xml';

function balancedVoucher(overrides: Partial<TallyVoucherInput> = {}): TallyVoucherInput {
  return {
    voucherType: 'Sales',
...
describe('escapeXml', () => {
  it('escapes the five XML special characters', () => {
    expect(escapeXml(`<a href="x">O'Brien & Sons</a>`)).toBe('&lt;a href=&quot;x&quot;&gt;O&apos;Brien &amp; Sons&lt;/a&gt;');
  });
});

describe('isVoucherBalanced', () => {
  it('is true when total debits equal total credits', () => {
    expect(isVoucherBalanced(balancedVoucher())).toBe(true);
  });
  ...
  it('is false when debits and credits differ', () => {
    ...
    expect(isVoucherBalanced(unbalanced)).toBe(false);
  });
});

describe('buildTallyVoucherXml', () => {
  it('produces a well-formed envelope containing every balanced voucher', () => {
    const { xml, skipped } = buildTallyVoucherXml([balancedVoucher()], 'Demo Workshop');
  ...
  it('excludes an unbalanced voucher from the XML and reports it as skipped, rather than sending Tally a bad envelope', () => {
    ...
    const { xml, skipped } = buildTallyVoucherXml([balancedVoucher(), unbalanced], 'Demo Workshop');
  ...
  it('escapes special characters in ledger/party names', () => {
    ...
    const { xml } = buildTallyVoucherXml([voucher], 'Demo Workshop');
