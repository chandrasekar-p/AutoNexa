...
function buildInvoicePdf(data) {
    return new Promise((resolve, reject) => {
        ...
        const totalsRow = (label, value, emphasize = false) => {
            doc
        ...
        if (Number(data.cgstAmount) > 0)
            totalsRow('CGST', money(data.cgstAmount));
        if (Number(data.sgstAmount) > 0)
            totalsRow('SGST', money(data.sgstAmount));
        if (Number(data.igstAmount) > 0)
            totalsRow('IGST', money(data.igstAmount));
        if (Number(data.roundOff) !== 0)
            totalsRow('Round Off', money(data.roundOff));
