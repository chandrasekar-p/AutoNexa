...
const gstr1_rows_1 = require("../src/modules/export/gstr1-rows");
...
describe('buildB2bRows', () => {
    it('includes only invoices with a customer GSTIN, same-state split into CGST+SGST', () => {
        const rows = (0, gstr1_rows_1.buildB2bRows)([invoice({ customerGstin: '33ABCDE1234F1Z5', customerState: 'Tamil Nadu' }), invoice({ customerGstin: null })], 'Tamil Nadu');
    ...
    it('splits into IGST for an inter-state customer', () => {
        const rows = (0, gstr1_rows_1.buildB2bRows)([invoice({ customerGstin: '27ABCDE1234F1Z5', customerState: 'Maharashtra' })], 'Tamil Nadu');
    ...
    it('produces one row per distinct rate within the same invoice', () => {
        const rows = (0, gstr1_rows_1.buildB2bRows)([
            invoice({
...
describe('buildB2cSummaryRows', () => {
    it('groups by place of supply and rate, ignoring B2B invoices', () => {
        const rows = (0, gstr1_rows_1.buildB2cSummaryRows)([invoice({ customerGstin: null, customerState: 'Tamil Nadu' }), invoice({ customerGstin: 'X' }), invoice({ customerGstin: null, customerState: 'Tamil Nadu' })], 'Tamil Nadu');
        expect(rows).toHaveLength(1);
...
describe('buildHsnSummaryRows', () => {
    it('buckets a line with no HSN/SAC under UNSPECIFIED rather than dropping it', () => {
        const rows = (0, gstr1_rows_1.buildHsnSummaryRows)([invoice({ lineItems: [{ hsnSac: null, gstRate: 18, lineTotal: 1000, quantity: 1 }] })], 'Tamil Nadu');
        expect(rows).toHaveLength(1);
    ...
    it('sums quantity and taxable value across B2B and B2C invoices sharing an HSN+rate', () => {
        const rows = (0, gstr1_rows_1.buildHsnSummaryRows)([
...
describe('countMissingHsn', () => {
    it('counts every line item across every invoice with no hsnSac', () => {
        const count = (0, gstr1_rows_1.countMissingHsn)([
    ...
    it('returns 0 when every line has an HSN/SAC code', () => {
        expect((0, gstr1_rows_1.countMissingHsn)([invoice()])).toBe(0);
    });
});
//# sourceMappingURL=gstr1-rows.spec.js.map